...
setup_nagios_user_and_groups() {
    ...
    run_command "sudo groupadd nagios || true" "No se pudo crear el grupo nagios"
    run_command "sudo groupadd nagcmd || true" "No se pudo crear el grupo nagcmd"
    run_command "sudo useradd -m -s /bin/bash -g nagios nagios || true" "No se pudo crear el usuario nagios"
...
download_and_install_nagios() {
    log_message "$GREEN" "Descargando e instalando Nagios Core y Plugins..."
    ...
    echo -e "${GREEN}Nombre de la Instancia de Nagios: ${RESET_COLOR}"
    read instance_name
...
setup_web_access() {
    ...
    run_command "sudo make install-webconf" "No se pudo instalar la configuración web"
